...
- [ ] **Git identity is not tracked in dotfiles.** `git/config` contains no email. Identity resolves
...
- [ ] **`.zshrc` has no environment-specific logic.** The `/workspace/.venv/bin/activate` line
...
- [ ] **Plugins live in a new `claude-plugins` repo.** `claude/plugins/` is removed from dotfiles.
...
- [ ] **`install.sh` clones/updates `claude-plugins` and points the `noah-local` marketplace at the
...
- [ ] **`secrets.zsh` stops hardcoding `facts.ontra.tools`.** The PIP index block reads the host
...
- [ ] **Nothing-breaks regression.** Running `install.sh` on this machine after the changes:
